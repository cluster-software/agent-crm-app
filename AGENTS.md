...
Generated workspace guidance for Claude/Codex is owned by `@agent-crm/sdk`, not
by this app. The app should load `AGENT_WORKSPACE_INSTRUCTIONS` from the SDK and
should not carry a long duplicated fallback.

When bumping `@agent-crm/sdk` or changing workspace creation, keep
`npm run check:agent-instructions` passing. If SDK/CLI behavior changes
upstream, update `packages/sdk/src/agent-instructions.ts` in the `agent-crm`
repo instead of copying new prose into this app.
...
Because release-please derives releases from merged PR titles, any PR that
should trigger a release must have a proper Conventional Commit title, such as
`fix: ...` or `feat: ...`. A vague or non-conventional PR title can prevent the
release workflow from cutting the expected release.
